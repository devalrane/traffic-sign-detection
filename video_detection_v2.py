...
def returnWhiteness(img):
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    return gray

def returnYellowness(img):
...
# Open the video file
video_file = 'Footage/shortened.mp4'
...
while True:
    ...
    whiteness = returnWhiteness(frame)
    yellowness = returnYellowness(frame)
    
    red_thresh = threshold(redness)    
    blue_thresh = threshold(blueness)
    white_thresh = threshold(whiteness)
    yellow_thresh = threshold(yellowness)
    
    try:
        red_contours = findContour(red_thresh)
        blue_contours = findContour(blue_thresh)
        white_contours = findContour(white_thresh)
        yellow_contours = findContour(yellow_thresh)
            
        red_big = findBiggestContour(red_contours)
        blue_big = findBiggestContour(blue_contours)
        white_big = findBiggestContour(white_contours)
        yellow_big = findBiggestContour(yellow_contours)
        
        if cv2.contourArea(blue_big) > 3000:
            img, sign = boundaryBox(frame, blue_big)
            label = label_to_text[predict(sign)]
            print(f"Now, I see a blue sign: {label}")
        elif cv2.contourArea(white_big) > 3000:
            img, sign = boundaryBox(frame, white_big)
            label = label_to_text[predict(sign)]
            print(f"Now, I see a white sign: {label}")
        elif cv2.contourArea(yellow_big) > 3000:
            img, sign = boundaryBox(frame, yellow_big)
            label = label_to_text[predict(sign)]
            print(f"Now, I see a yellow sign: {label}")
        elif cv2.contourArea(red_big) > 3000:
